namespace Practica03 {
    class Ejercicio2 {
        ...
        public void Menu () {
            ...
            Console.Write("¿Desea solicitar servicio a domicilio? [1.-SI|2.-NO]: ");
            int opcDomicilio = Convert.ToInt32(Console.ReadLine());

            double precioPizza = PRECIOS[opcPizza-1];
            double cargoAdicional = opcPago == 1 ? precioPizza * CARGO_TARJETA : 0.0;
            double servicioDomicilio = opcDomicilio == 1 ? SERVICIO_DOMICILIO : 0.0;
            
            Console.WriteLine($"\nPizza: ${precioPizza}");
            if (cargoAdicional > 0)
                Console.WriteLine($"Cargo adicional por pago tarjeta: ${cargoAdicional}");
            if (servicioDomicilio > 0)
                Console.WriteLine($"Servicio a domicilio: ${servicioDomicilio}");
            Console.WriteLine($"El cliente debe de pagar ${(precioPizza+cargoAdicional+servicioDomicilio)}");
        }
    }
}